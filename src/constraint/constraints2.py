...
def prerequisites_violation_penalty(plan,uni_program_list,max_sems):
    ...
    for i in range(len(plan)):
        for j in range(len(plan[i])):
            sem_id = plan[i][j]
            if sem_id != 0:
                presented_course_plan[sem_id-1].append(j)

    for program in uni_program_list:
        course_list = program.courses
        for i in len(presented_course_plan):
            semester = presented_course_plan[i]
...
def corequisites_violation_penalty(plan,uni_program_list,max_sems):
    penalty = 0
    presented_course_plan=[[]] * max_sems
    for i in range(len(plan)):
        for j in range(len(plan[i])):
    ...
    for program in uni_program_list:
        course_list = program.courses
        for semester in presented_course_plan:
            for course_id in semester:
...
def plan_total_load_violation(plan,uni_program_list,max_sems):
    penalty = 0
    presented_course_plan=[[]] * max_sems
    for i in range(len(plan)):
        for j in range(len(plan[i])):
...
def student_load_violation(plan,uni_program_list,student_list,max_sems):
    penalty = 0
    ...
    for student in student_list:
        ...
        for course in student_program_courses:
            ...
            if total_plan_load != (student_program_mandatory_load+student_program_elective_load):
                penalty+=1
    return penalty
def total_violation(plan,uni_program_list,student_list,max_sems):
    violation = 0
    violation += prerequisites_violation_penalty(plan,uni_program_list,max_sems)
    violation += corequisites_violation_penalty(plan,uni_program_list,max_sems)
    violation += plan_total_load_violation(plan,uni_program_list,max_sems)
    violation += student_load_violation(plan,uni_program_list,student_list,max_sems)
    return violation
